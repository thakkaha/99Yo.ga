...
// Add '1yo.ga/' prefix to input#custom-url
  ...
  if (validateURL(longURL)) {
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 201) {
        ...
        returnedShortURL = "1yo.ga/" + data.short;
        displayOutput(
          returnedShortURL,
    ...
    request =
      "https://thakkaha.dev.fast.sheridanc.on.ca/1pt/addURL-2.php?url=" +
      longURL +
      "&cu=" +
      encodeURI(shortURL);
...
submit.onclick = function () {
  sendRequest(
    input.value,
    remove(customURLInput.value, ["1yo.ga/", "/", "\\?"])
  );
};
